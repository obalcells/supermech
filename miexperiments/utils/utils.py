...
def line(y: Union[torch.Tensor, List[torch.Tensor]], renderer=None, **kwargs):
    '''
...
def bar(tensor, renderer=None, **kwargs):
    ...
    if isinstance(tensor, list):
        if isinstance(tensor[0], torch.Tensor): arr = [to_numpy(tn) for tn in tensor]
...
def plot_comp_scores(model: HookedTransformer, comp_scores, title: str = "", baseline: Optional[torch.Tensor] = None) -> go.Figure:
    px.imshow(
...
def plot_logit_attribution(model: HookedTransformer, logit_attr: torch.Tensor, tokens: torch.Tensor, title: str = ""):
...
def plot_neurons(neurons_in_unbalanced_dir: Float[Tensor, "batch neurons"], model: HookedTransformer, data, failure_types_dict: Dict, layer: int, renderer=None):
    ...
    neuron_numbers = einops.repeat(torch.arange(model.cfg.d_model), "n -> (s n)", s=data.starts_open.sum())
...
def plot_cosine_sim(a: Float[Tensor, "len_a vector_dim"], b: Float[Tensor, "len_b vector_dim"]):
    dot_product_matrix = einops.einsum(
        normalize(a, p=2.0), normalize(b, p=2.0),
        "len_a vector_dim, len_b vector_dim -> len_a len_b"
    )

    imshow(
        dot_product_matrix, 
        labels={"x": "j", "y": "i", "color": "Dot produdct"},
        title=f"Cosine similarity matrix", 
        text_auto=".2f",
        width=600, height=600,
    )

def plot_dot_product(a: Float[Tensor, "len_a vector_dim"], b: Float[Tensor, "len_b vector_dim"]):
    # the same as above but without normalizing
    dot_product_matrix = einops.einsum(
        a, b,
        "len_a vector_dim, len_b vector_dim -> len_a len_b"
    )
...
def plot_top_k_tokens(tokens: List[str], logits: List[Float], topk=10):
    data = {
        f"Top {topk} output tokens": (tokens, logits)
    }
