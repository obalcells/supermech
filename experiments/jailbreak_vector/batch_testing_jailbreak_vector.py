...
def tokenize_and_batch(instructions, tokenizer=tokenizer, batch_size=32):
    prompts = [instruction_to_prompt(instruction) for instruction in instructions]
...
def batch_generate_with_hooks(
    ...
    all_toks = torch.zeros((batch_size, toks.shape[1] + max_tokens_generated), dtype=torch.long).to(device)
    all_toks[:, :toks.shape[1]] = toks

    tl_model.reset_hooks()
    
    for i in tqdm.tqdm(range(max_tokens_generated), disable=disable_tqdm):
        attention_mask = (all_toks[:,:-max_tokens_generated+i] != 0).to(batch.device)
        ...
        )[:,-1] # get the first element in the batch, and the last logits tensor in the sequence

        next_tokens = logits.argmax(dim=-1) 
        all_toks[:,-max_tokens_generated+i] = next_tokens

    if include_prompt:
        return [tokenizer.decode(all_toks[i]) for i in range(batch_size)]
    else:
        return [tokenizer.decode(all_toks[i, toks.shape[1]:]) for i in range(batch_size)]

def fast_add_vec_after_inst_hook_fn(
    resid_post: Float[Tensor, "batch_size seq_len d_model"],
    hook: HookPoint,
    vec: Float[Tensor, "n_layers d_model"],
    input_ids: Int[Tensor, "batch_size seq_len"], # padded from the left
    layers_to_steer: Union[int, List[int]],
    normalize: bool=True,
    multiplier: float=1.0,
):
    if isinstance(layers_to_steer, int):
        layers_to_steer = [layers_to_steer]

    layer = hook.layer()

    batch_size = resid_post.shape[0]
    tokens_generated = resid_post.shape[1] - input_ids.shape[1]
    inst_token_position = -1 - tokens_generated

    # we subtract the steering vector we added in the previous layer
    # but only if it's not the last layer that we were going to add the steering vector to
    if layer-1 in layers_to_steer and layers_to_steer[-1] != layer-1:
        norm_pre = torch.norm(resid_post, dim=-1, keepdim=True)
        resid_post[:,inst_token_position:] = resid_post[:,inst_token_position:] - vec[layer-1] * multiplier
        norm_post = torch.norm(resid_post, dim=-1, keepdim=True)
        resid_post = resid_post / norm_post * norm_pre

    if not layer in layers_to_steer:
        return resid_post
...
# batch = tokenize_and_batch(testing_steering_instructions)[0]

batch = tl_model.to_tokens(testing_steering_instructions, padding_side='left')
...
for layer in layers:
    ...
    for normalize in normalize_options:
        for multiplier in tqdm.tqdm(multipliers):
            tmp_hook_fn = functools.partial(
                fast_add_vec_after_inst_hook_fn,
                vec=steering_vector_at_last_pos,
                input_ids=batch,
            ...
            model_outputs = batch_generate_with_hooks(
                batch,
                fwd_hooks=[(hook_filter, tmp_hook_fn)],
                max_tokens_generated=64
            )
            ...
            answers = []
            for instruction, model_output in zip(testing_steering_instructions, model_outputs):
                answers.append({"instruction": instruction, "answer": model_output})
                print(f"{instruction} -> {model_output}")
    ...
    # we dump it at every layer in case the notebook stops for some reason
    with open("fast_results.json", "w") as jfile:
        json.dump(results, jfile)
